"""
CaptionTester3 - Multiple words on screen, current word highlighted in yellow.

Shows accumulating words (e.g., "Hello world how") with the current word in yellow,
previous words in white.

Usage:
    python caption_tester_multiword.py <audio_file> [output_video]
    python caption_tester_multiword.py --from-config
    python caption_tester_multiword.py   (opens GUI)
"""
import os
import subprocess
import sys
...
def generate_ass_subtitle(result) -> str:
    """
    Multiple words on screen: previous words white, current word yellow.
    Each Dialogue shows: "word1 word2 word3" with word3 in yellow.
    """
    ass_content = ""
    MIN_WORD_DUR = 1.2
    ...
    # ASS colors: \c&HBBGGRR&. Yellow=BGR 00FFFF, White=FFFFFF
    YELLOW = "00FFFF"   # current word (B=0,G=255,R=255)
    WHITE = "FFFFFF"    # previous words
    ...
    for segment in result["segments"]:
        ...
        for i, word_info in enumerate(words):
            ...
            # Build text: previous words (white) + current word (yellow)
            prev_words = " ".join(w.get("word", "").strip() for w in words[:i] if w.get("word", "").strip())
            if prev_words:
                line_text = f"{prev_words} {{\\c&H{YELLOW}}}{word_text}{{\\c&H{WHITE}}}"
            else:
                line_text = f"{{\\c&H{YELLOW}}}{word_text}{{\\c&H{WHITE}}}"

            start = format_time(start_time)
            end = format_time(end_time)
            ass_content += f"Dialogue: 0,{start},{end},Default,,0,0,0,,{{\\fad({FADE_IN_MS},{FADE_OUT_MS})}}{line_text}\n"
...
def build_ass_file(ass_content: str) -> str:
    header = """[Script Info]
Title: CaptionTester3 - Multi-word Highlight
ScriptType: v4.00+
PlayResX: 1920
PlayResY: 1080
...
def run(audio_path: str, output_path: str | None = None, model_size: str = "base", log=None) -> str:
    ...
    if output_path is None:
        ...
        output_path = str(Path(audio_path).parent / f"{base}_multiword.mp4")
    ...
    _log("Generating ASS (multi-word, current in yellow)...")
    ass_events = generate_ass_subtitle(result)
    ass_content = build_ass_file(ass_events)

    fd, ass_path_str = tempfile.mkstemp(suffix=".ass", prefix="captiontester3_")
...
def main():
    CONFIG_PATH = Path(__file__).parent / "caption_tester3_config.json"
    ...
    if "--from-config" in sys.argv or "-c" in sys.argv:
        try:
            ...
            if config_file.exists():
                ...
                if output and "_multiword" not in output:
                    output = str(Path(output).with_suffix("")) + "_multiword.mp4"
                if audio and output and os.path.isfile(audio):
                    run(audio, output)
                    return
            print("No config. Run CaptionTester GUI first.")
    ...
    if len(sys.argv) >= 2 and not sys.argv[1].startswith("-"):
        ...
        root = tk.Tk()
        root.title("CaptionTester3 - Multi-word Highlight")
        ...
        def browse_audio():
            ...
            if path:
                ...
                if not output_var.get():
                    output_var.set(str(Path(path).parent / f"{Path(path).stem}_multiword.mp4"))
        ...
        def run_task():
            ...
            def do_work():
                try:
                    ...
                    root.after(0, lambda m=str(e): messagebox.showerror("Error", m))
                finally:
        ...
        run_btn = ttk.Button(f, text="Run (multi-word, current in yellow)", command=run_task)
